...
(function() {
  ...
  if (window.location.hostname.includes('khanacademy.org')) {
    ...
    // Inject external script file to avoid CSP issues
    const script = document.createElement('script');
    script.src = chrome.runtime.getURL('inject.js');
    script.onload = function() {
      console.log('✅ Injection script loaded successfully');
      this.remove();
    };
    ...
    window.addEventListener('message', (event) => {
      if (event.data && event.data.type === 'KHAN_ANSWERS_QUEUED') {
        ...
        const formFields = [{
          ...
          value: "",
          options: [],
          instructions: "CRITICAL: You MUST solve this math problem correctly step by step.\n\nFor multiplication:\n- 70 × 3,000 = 70 × 3000 = 210,000 (NOT 8!)\n- 50 × 400 = 20,000\n- Always multiply ALL digits\n\nFor division:\n- Check your work by multiplying back\n- Include decimal points where needed\n\nRETURN ONLY THE NUMERIC ANSWER with no units, commas, or extra text."
        }];
        
        // Call Gemini API for the first question (no screenshot needed)
        callGemini(formFields, null).then(result => {
          ...
          if (result && Array.isArray(result) && result.length > 0) {
            console.log('✅ Gemini responded with answer:', result);
            // Extract just the answer value
            const answer = result[0].value;
            ...
            console.error('❌ Gemini failed to respond properly:', result);
            showNotification('❌ Gemini AI failed to process the question', 'error');
          }
        }).catch(error => {
