...
Example: [{"label": "Name", "value": "Alex Johnson"}]` },
  ...
  async function fillGoogleFormField(field, value) {
    ...
    switch (field.type) {
      ...
      case 'email':
        ...
        // Method 1: Direct value setting with comprehensive events
        input.focus();
        input.click();
        ...
        // Method 2: Use different approach for longer text
        if (valueStr.length > 10) {
          console.log('🔄 Using enhanced approach for longer text...');
          
          // Clear the field completely
          input.value = '';
          input.select();
          
          // For longer text, use a slower, more reliable approach
          for (let i = 0; i < valueStr.length; i++) {
            input.value = valueStr.substring(0, i + 1);
            
            // Fire input event after each character for Google Forms
            ...
            // Small delay for Google Forms to process
            if (i % 10 === 0) { // Every 10 characters
              await new Promise(resolve => setTimeout(resolve, 5));
            }
          }
        } else {
          // For shorter text, use standard approach
          try {
            input.select();
            document.execCommand('insertText', false, valueStr);
          } catch (e) {
            console.log('execCommand not available, using direct input');
          }
        }
        
        // Method 3: Simulate actual typing for stubborn Google Forms
        const events = [
        ...
        // Method 4: Force update the underlying React/Angular state if present
        ...
        if (reactKey) {
          ...
          if (reactInstance && reactInstance.memoizedProps) {
            if (reactInstance.memoizedProps.onChange) {
              reactInstance.memoizedProps.onChange({ target: { value: valueStr } });
            }
          }
        }
        
        // Method 5: Try setting via prototype
        ...
        if (input.value !== valueStr) {
          console.log(`⚠️ Text not saved properly. Expected: "${valueStr}", Got: "${input.value}". Length difference: ${valueStr.length - input.value.length}`);
          
          // Ultimate fallback - force the value in multiple ways
          input.focus();
          
          // Method 1: Use native setter with comprehensive events
          ...
          // Method 2: If still not working, try DOM manipulation
          if (input.value !== valueStr) {
            console.log('🔥 Using DOM manipulation...');
            input.setAttribute('value', valueStr);
            input.defaultValue = valueStr;
            
            // Try to trigger React if present
            const reactFiber = input._reactInternalFiber || input._reactInternalInstance;
            if (reactFiber) {
              console.log('🚨 Triggering React update...');
  ...
  async function sendSubmitPatternToBackend(pattern) {
    try {
      const response = await fetch('https://ai-form-solver-extension.replit.app/api/submit-patterns', {
        method: 'POST',
        headers: {
  ...
  function startLearningMode() {
    ...
    document.addEventListener('click', function learnClick(e) {
      ...
      if (isSubmit) {
        ...
        // Store button info
        chrome.storage.sync.get(['learnedButtons'], (result) => {
          ...
          chrome.storage.sync.set({ learnedButtons: buttons });
          
          // Send to backend for learning compilation
          sendSubmitPatternToBackend({
        ...
        document.removeEventListener('click', learnClick);
