  ...
  async function fillGoogleFormField(field, value) {
    ...
    switch (field.type) {
      ...
      case 'email':
        ...
        // Focus and click to activate the field
        input.focus();
        input.click();
        await new Promise(resolve => setTimeout(resolve, 100));
        ...
        // For longer text, type in chunks to avoid issues
        if (valueStr.length > 50) {
          // Type in chunks of 10 characters for better reliability
          for (let i = 0; i < valueStr.length; i += 10) {
            const chunk = valueStr.slice(i, i + 10);
            input.value += chunk;
            input.dispatchEvent(new Event('input', { bubbles: true }));
        ...
        // Multiple events to ensure Google Forms saves the value
        input.dispatchEvent(new Event('input', { bubbles: true }));
        input.dispatchEvent(new Event('change', { bubbles: true }));
        input.dispatchEvent(new KeyboardEvent('keyup', { bubbles: true, key: 'Enter' }));
        
        // Wait a bit before blur to ensure value is saved
