  ...
  async function fillGoogleFormField(field, value) {
    ...
    switch (field.type) {
      ...
      case 'email':
        ...
        // Simplified approach - set value and fire essential events only
        input.value = valueStr;
        
        // Use native setter for Google Forms compatibility  
        const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
        if (nativeInputValueSetter) {
          nativeInputValueSetter.call(input, valueStr);
        }
        
        // Fire essential events for Google Forms validation
        input.dispatchEvent(new Event('input', { bubbles: true, composed: true }));
        input.dispatchEvent(new Event('change', { bubbles: true }));
        
        // For longer text, add slight delay and double-check
        if (valueStr.length > 10) {
          await new Promise(resolve => setTimeout(resolve, 100));
          
          // Verify and retry once if needed
          if (input.value !== valueStr) {
            console.log('⚠️ Retrying text input...');
            input.value = valueStr;
            nativeInputValueSetter.call(input, valueStr);
