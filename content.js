  ...
  async function fillGoogleFormField(field, value) {
    ...
    switch (field.type) {
      ...
      case 'email':
        ...
        // Method 2: Try clipboard approach for Google Forms
        try {
          // Clear and paste the text
          input.select();
          ...
          console.log('Clipboard/execCommand not available, using direct input');
        }
        ...
        // Final verification and retry with multiple approaches
        await new Promise(resolve => setTimeout(resolve, 300));
        
        if (input.value !== valueStr) {
          console.log(`⚠️ Text not saved properly. Expected: "${valueStr}", Got: "${input.value}". Trying all approaches...`);
          
          // Approach 1: Clear and type slowly
          input.focus();
          input.select();
          document.execCommand('delete');
          ...
          for (let i = 0; i < valueStr.length; i++) {
            ...
            // Fire multiple events for each character
            input.dispatchEvent(new KeyboardEvent('keydown', { bubbles: true, key: char }));
            input.dispatchEvent(new Event('input', { bubbles: true, data: char }));
            input.dispatchEvent(new KeyboardEvent('keyup', { bubbles: true, key: char }));
            
            await new Promise(resolve => setTimeout(resolve, 10));
          }
          
          // Approach 2: Force through React if still not working
          if (input.value !== valueStr) {
            console.log('🚨 Trying React bypass...');
            const descriptor = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value');
            descriptor.set.call(input, valueStr);
          ...
          // Approach 3: Last resort - manual DOM manipulation
          if (input.value !== valueStr) {
            console.log('🔥 Last resort: Manual DOM manipulation...');
