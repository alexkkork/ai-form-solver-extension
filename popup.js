...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('openAdmin').addEventListener('click', async () => {
    try {
      // Get current Replit URL from the active tab or use default
      const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
      let adminUrl = 'https://ai-form-solver.replit.app/admin';
      
      // If we're on a Replit domain, use that domain
      ...
      showStatus('Opening admin panel...', 'success');
    } catch (error) {
      console.error('Error opening admin panel:', error);
      showStatus('Error opening admin panel', 'error');
    }
  });
  ...
  document.getElementById('adminLoginBtn').addEventListener('click', async () => {
    ...
    if (passcode === 'muddyglass29') {
      ...
      window.open(chrome.runtime.getURL('admin.html'), '_blank');
    } else {
  ...
  document.getElementById('reportError').addEventListener('click', async () => {
    ...
    try {
      ...
      const response = await fetch('https://ai-form-solver.replit.app/api/error-reports', {
        method: 'POST',
        headers: {
      ...
      showStatus('Failed to collect error data', 'error');
    }
  });

  // Check if user has admin access
  chrome.storage.sync.get(['isAdmin'], (result) => {
    if (result.isAdmin) {
      document.getElementById('reportError').style.display = 'block';
    }
  });
