...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Set saved toggle states
  if (settings.autoNavigate !== undefined) {
    document.getElementById('autoNavigate').checked = settings.autoNavigate;
  }
  if (settings.khanMode !== undefined) {
    document.getElementById('khanModeToggle').checked = settings.khanMode;
  } else {
    // Default to Khan Mode enabled
    document.getElementById('khanModeToggle').checked = true;
  }
  ...
  document.getElementById('solveForm').addEventListener('click', async () => {
    ...
    // Check if API key is still empty
    if (!apiKey) {
    ...
    const khanMode = document.getElementById('khanModeToggle').checked;
    await chrome.storage.sync.set({ autoNavigate, khanMode });
  ...
  // Auto Submit button
  document.getElementById('autoSubmit').addEventListener('click', async () => {
    showStatus('Scanning for submit buttons...', 'info');
    ...
    try {
      ...
      if (response && response.success) {
        showStatus(response.message || 'Submit action completed!', 'success');
      } else {
        showStatus(`Error: ${response ? response.error : 'No response from content script'}`, 'error');
      }
    } catch (error) {
